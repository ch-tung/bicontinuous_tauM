...
for T_i = fliplr(Temperature) % loop over temperature
    ...
    for ic = 1:n_conf % loop over configuration
        ...
        for it = index_time
            ...
            n_edge = 1;
            [XX, YY, ZZ] = meshgrid((-n_edge*dl:dl:Lx+n_edge*dl)-Lx/2,(-n_edge*dl:dl:Ly+n_edge*dl)-Ly/2,0);
...
%             % contour_blue
%             contourf(XX, YY, CC, [0:-1/3:-1]*abs(min(CC,[],'all')),'-w');
%             % contour_yellow
%             contourf(XX, YY, CC, [0:1/3:1]*abs(max(CC,[],'all')),'-k');
            
            shading interp
            
            % outline
            contour(XX, YY, CC, [-100 0 100],'-k','LineWidth',2);
