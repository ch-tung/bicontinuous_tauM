...
for iT in range(n_T):
    ...
    for ic in range(n_conf):
        ...
        for it in range(n_t):
            ...
            Cv = (Cv*3-1)/2
            Cv_CG = CG(rt, L, rc, sl, Cv)
            
            C_0[it] = np.average(Cv_CG[(Cvz_rg[:,2]>=0),2])
            C_1[it] = np.average(Cv_CG[(Cvz_rg[:,2]<0),2])
            C[it] = np.average(Cv_CG[:,2])
    ...
    filename_dcorr = 'tSD_CG_vo_py_' + name_T + '.mat'
