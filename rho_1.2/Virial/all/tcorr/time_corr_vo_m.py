...
for iT in range(n_T):
    ...
    for ic in range(n_conf):
        ...
        v_all_0 = v_all[:,:,it]
        W_0, V_0 = calculate_shape(v_all_0,n_particle)
        ...
        for it in range(n_t):
            ...
            vm_0_0 = np.average(v_all_0[Cvz_rg[:,2]>=0,:],0)
            vm_0_1 = np.average(v_all_0[Cvz_rg[:,2]<0,:],0)
            vm_t_0 = np.average(v_all_t[Cvz_rg[:,2]>=0,:],0)
            vm_t_1 = np.average(v_all_t[Cvz_rg[:,2]<0,:],0)
            
            Wm_0_0, Vm_0_0 = calculate_shape(vm_0_0.reshape((1,6)),1)
            Wm_0_1, Vm_0_1 = calculate_shape(vm_0_1.reshape((1,6)),1)
            Wm_t_0, Vm_t_0 = calculate_shape(vm_t_0.reshape((1,6)),1)
            Wm_t_1, Vm_t_1 = calculate_shape(vm_t_1.reshape((1,6)),1)
            Vm_0_0 = np.transpose(Vm_0_0,[0, 2, 1])
            Vm_t_0 = np.transpose(Vm_t_0,[0, 2, 1])
            Vm_0_1 = np.transpose(Vm_0_1,[0, 2, 1])
            Vm_t_1 = np.transpose(Vm_t_1,[0, 2, 1])
            pm0_0 = np.sum(vm_0_0[0:3])/3
            pm0_1 = np.sum(vm_0_1[0:3])/3
            sm0_0 = vm_0_0[3:6]
            sm0_1 = vm_0_1[3:6]
            pmt_0 = np.sum(vm_t_0[0:3])/3
            pmt_1 = np.sum(vm_t_1[0:3])/3
            smt_0 = vm_t_0[3:6]
            smt_1 = vm_t_1[3:6]
            ...
            for iV in range(3):
                V_0_i = Vm_0_0[:,:,iV];
                V_i = Vm_t_0[:,:,iV];
                Cv_0[iV] = np.sum(np.multiply(V_0_i,V_i),0)**2
                
                V_0_i = Vm_0_1[:,:,iV];
                V_i = Vm_t_1[:,:,iV];
                Cv_1[iV] = np.sum(np.multiply(V_0_i,V_i),0)**2
        ...
        C_0_c = C_0_c + C_0
        C_1_c = C_1_c + C_1
        Var_s = Var_s + np.average((s0-Ms0)**2)
        
    C_0_c = C_0_c/n_conf
    C_1_c = C_1_c/n_conf
    Var_s = Var_s/n_conf
    
    filename_dcorr = 'tcorr_m_vo_py_' + name_T + '.mat'
