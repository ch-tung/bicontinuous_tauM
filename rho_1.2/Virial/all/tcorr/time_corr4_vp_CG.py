...
for iT in range(n_T):
    ...
    for ic in range(n_conf):
        ...
        for it in range(n_t):
            ...
            iV = 2
            sim = Cpz.reshape(n_particle,1)*Cpz.reshape(1,n_particle)
            
            mean_Cp[1,it] = np.average(Cpz[index_Cvz_1[:,0]])
            mean_Cp[0,it] = np.average(Cpz[index_Cvz_0[:,0]])
    ...
    filename_dcorr = 'tcorr4_CG_vp_py_' + name_T + '.mat'
