...
for T_i = fliplr(Temperature) % loop over temperature
    ...
    for ic = 1:n_conf % loop over configuration
        ...
        for it = index_time
            ...
            [XX, YY, ZZ] = meshgrid((-6:0.1:6),(-6:0.1:6),(-6:0.1:6));
...
%             set(gca, 'Projection','perspective')
            ...
            color_RWB = [[ones(1,129); 0:1/128:1; 0:1/128:1]'; [1:-1/128:0; 1:-1/128:0; ones(1,129)]'];
            MC_F = mean(MC(f),2);
            MC_transform = MC;
            MC_transform(MC>2) = 2;
            MC_transform(MC<-2) = -2;
            ...
            color_MC = color_RWB(ceil((MC_transform + 2)/4*257)+1,:);

            p.EdgeColor = 'none';
            p.FaceColor = 'interp';
            p.FaceVertexCData = color_MC;
