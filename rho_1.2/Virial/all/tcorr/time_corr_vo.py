...
for iT in range(n_T):
    ...
    CV_0 = np.zeros(n_frame)
    CV_1 = np.zeros(n_frame)
    ...
    for ic in range(n_conf):
        ...
        V_0 = np.transpose(V_0,[0, 2, 1]) # stress eigenvector
        ...
        for it in range(n_t):
            ...
            V = np.transpose(V,[0, 2, 1]) # stress eigenvector
            ...
            CV_0[it] = np.average(Cvz[Cvz_rg[:,2]>=0,2],0)
            CV_1[it] = np.average(Cvz[Cvz_rg[:,2]<0,2],0)
    ...
    filename_dcorr = 'tcorr_vo_py_' + name_T + '.mat'
