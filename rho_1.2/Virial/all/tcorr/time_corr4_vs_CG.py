...
for iT in range(n_T):
    ...
    for ic in range(n_conf):
        ...
        for it in range(n_t):
            ...
            iV = 2
            sim = Csz.reshape(n_particle,1)*Csz.reshape(1,n_particle)
            
            mean_Cp[1,it] = np.average(Csz[index_Cvz_1[:,0]])
            mean_Cp[0,it] = np.average(Csz[index_Cvz_0[:,0]])
    ...
    filename_dcorr = 'tcorr4_CG_vs_py_' + name_T + '.mat'
    mdic = {'sim_r_11_c':sim_r_11_c, 'sim_r_10_c':sim_r_10_c, 'sim_r_00_c':sim_r_00_c, 'sim_q_c':sim_q_c, 'sim_s_c':sim_s_c, 'sim_a_c':sim_a_c, 'rr':rr, 'qq':qq, 't':t, 'mean_Cp_c':mean_Cp_c}
