...
index_color = round(index_time/length(t)*100);
...
%     [min_dtau_M, index_time] = min(abs(t-tau_M(Temperature==T_i)));
    ...
    Pq_c = zeros(n_grid/2,length(index_time));
...
%         l = load('../l.txt');
        ...
        itime = 0;
        Pq = zeros(n_grid/2,length(index_time));
        %% loop over time
        for it = index_time
            disp(['t = ', num2str(t(it))])
            itime = itime+1;
...
%             SP((XX.^2+YY.^2+ZZ.^2)<5) = 1;
            ...
            SP_FFT = fftn(CC,[n_grid,n_grid,n_grid]);
            
            abs_SP_FFT = abs(SP_FFT).^2/var(CC(:));
...
%             caxis([0 1])
            ...
            dq = 2*pi/Lx;
            qq = dq*(1:n_grid/2)-dq/2;
            
            index_q = ceil(q/dq);
                       
            for iq = 1:n_grid/2
                vq = 4*pi*qq(iq)^2*dq/8;
                Pq(iq,itime) = sum(abs_SP_FFT(index_q==iq))/vq/(N_s^2/2)*2;
...
%     end
    ...
    for i_t = 1:length(index_time)
        plot(qq,Pq_c(:,i_t),'-','Color',color(index_color(i_t),:),'LineWidth',2)
    end
    ...
    %%
    figure(2)
    [XX,YY] = meshgrid(qq,t(index_time)/1000);
    pcolor(XX,YY,Pq_c')
    shading interp
    set(gca, 'YScale', 'log')
