...
for iT in range(n_T):
    ...
    for ic in range(n_conf):
        ...
        for it in range(n_t):
            ...
            i_sort = np.argsort(Cvz_rg[:,2])
            
            n_select = np.int(n_particle/10)
            
            C_0[it] = np.average(SD_CG[i_sort[-n_select:]])
            C_1[it] = np.average(SD_CG[i_sort[0:n_select]])
            C[it] = np.average(SD_CG)
            
        C_0_c = C_0_c + C_0
        C_1_c = C_1_c + C_1
        C_c = C_c + C
        Var_s = Var_s + np.average((p0-np.average(p0))**2)
    ...
    filename_dcorr = 'tSD_CG_ex_vp_py_' + name_T + '.mat'
    mdic = {'C_0':C_0_c, 'C_1':C_1_c, 'C':C_c, 't':t, 'Var_s':Var_s}
